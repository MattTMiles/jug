...
def _jump_delay_change(xp, params, setup):
    """Delay change from fitted JUMP parameters relative to the setup reference.

    Tempo2 JUMPs enter spin phase as ``phase += F0 * JUMP`` (seconds).  The
    Taylor delay overlay uses ``residual_delta ≈ -delay_change`` at first order,
    so a positive JUMP increment requires ``delay_change = -ΔJ`` on masked TOAs.
    """
    from jug.model.parameter_spec import is_jump_param

    jump_params = [p for p in (setup.fit_param_list or []) if is_jump_param(p)]
    if not jump_params and setup.jump_masks:
        jump_params = [p for p in setup.jump_masks if is_jump_param(p)]
    if not jump_params:
        return None

    tdb_mjd = xp.asarray(setup.tdb_mjd, dtype=xp.float64)
    delay_change = xp.zeros_like(tdb_mjd, dtype=xp.float64)
    for name in jump_params:
        new_val = xp.asarray(pval(params, name, 0.0), dtype=xp.float64)
        ref_val = xp.asarray(pval(setup.params, name, 0.0), dtype=xp.float64)
        delta_j = new_val - ref_val
        if setup.jump_masks and name in setup.jump_masks:
            mask = xp.asarray(setup.jump_masks[name], dtype=xp.float64)
        else:
            mask = xp.ones_like(tdb_mjd, dtype=xp.float64)
        delay_change = delay_change + mask * (-delta_j)
    return delay_change


def compute_side_delay_change(params, setup, *, xp):
    """Modular non-geometry delay changes: FD, FDJUMP, DMX, SW, JUMP.

    Excludes DM (tempo2 native BCLT), astrometry (native BBAT), and binary
    (``binary_plan`` on the tempo2 JAX path).  Safe to add on all tempo2 graph
    modes without double-counting geometry or dispersion from the native tail.
    """
    tdb_mjd = xp.asarray(setup.tdb_mjd, dtype=xp.float64)
    ...
    jump_part = _jump_delay_change(xp, params, setup)
    if jump_part is not None:
        delay_change = delay_change + jump_part
...
def compute_total_delay_change(params, setup, *, xp, binary_plan=None):
    ...
    delay_change = delay_change + compute_side_delay_change(params, setup, xp=xp)
