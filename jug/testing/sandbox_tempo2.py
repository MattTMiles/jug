...
def _count_tim_toas(timfile: str | Path) -> int:
    """Lightweight Tempo2 TIM TOA counter used for optional maxobs sizing."""
    count = 0
    skip = {"FORMAT", "MODE", "INCLUDE", "EFAC", "EQUAD", "JUMP", "TIME", "T2CMETHOD"}
    for raw_line in Path(timfile).read_text(errors="ignore").splitlines():
        line = raw_line.strip()
        if not line or line.startswith("#") or line.startswith("C "):
            continue
        parts = line.split()
        if not parts or parts[0].upper() in skip or len(parts) < 4:
            continue
        try:
            float(parts[1])
            float(parts[2])
...
class Policy:
    ...
    auto_nobs_retry: bool = False  # automatically add nobs parameter for large TOA files
    nobs_threshold: int = (
...
def _resolve_worker_cmd(env_name: Optional[str]) -> Tuple[List[str], bool]:
    ...
    )  # Go up from jug/testing/sandbox_tempo2.py to the JUG package root
    ...
    def python_to_worker_cmd(python_exe: str) -> List[str]:
        ...
        return [
            ...
            (
                ...
                f"import jug.testing.sandbox_tempo2 as m; "
...
class tempopulsar:
    ...
    def _proactive_nobs_setup(self):
        ...
        try:
            ...
            toa_count = _count_tim_toas(timfile_path)
