...
*Last updated: 2026-07-09 (JAX native chain gates on `tempo2-compat` through `54c4148`)*
...
`make_residual_delta_jax_fn()` (used by `export_jax_timing_state`, notebook
integrators, and `design_matrix_method="autodiff"`) evaluates a **local residual
...
Absolute native residuals use `phase5@bbat` with tempo2 binary orbit time in
`obsn[i].torb`. For diagnostic `Tempo2Terms.torb_sec`, JUG exports the host delay
ledger value `prebinary_delay_sec − total_delay_sec` (matches pytempo `torb_sec`).
Do **not** reconstruct `torb` from Taylor `dt_sec` and JAX `bbat` — that closure
carried a uniform ~60 ns bias on wsrt167.

With host `dt_emit` frozen in the fit setup, the spin closure
`torb = dt_emit − (bbat − PEPOCH)` would cancel delay changes in
`res(θ+Δθ) − res(θ)` when geometry/DM move `bbat`. The fitting tangent
...
why the tempo2 chain exposes the `fixed_state_bclt / fixed_state_stripped /
staged_bclt / full` graph modes (§5). The PINT path has nothing analogous to freeze.
...
When an external integrator binds JUG as the pint engine, NUTS sees JUG's fixed-state
...
For JAX-traced fitting (`compatibility="tempo2"`), the traced graph uses one of four
...
### Four tempo2-native graph modes
...
```mermaid
flowchart TB
  Mode["tempo2_native kwarg"] --> FixedBclt["fixed_state_bclt"]
  Mode --> Stripped["fixed_state_stripped"]
  Mode --> Staged["staged_bclt (default)"]
  Mode --> Full["full"]

  FrozenState["tempo2_obs_state + host ephemeris/clocks"] --> FixedBclt
  FrozenState --> Stripped
  FrozenState --> Staged
  Tables["SPK, EOP, IFTE, clocks in XLA"] --> Full

  FixedBclt --> OnePass["one-pass BCLT + full tempo2 tail\n(frozen dt_ssb_ref)"]
  Stripped --> Lite["BBAT lite subgraph\n(single pert vs host ref BBAT)"]
...
host `dt_sec_cached` fixed while recomputing `bbat` from perturbed BCLT/formBats.
A spin closure `torb = dt_emit − (bbat − PEPOCH)` would cancel delay motion in
the forward `phase5@bbat` value — giving **zero geometry columns** even when
autodiff is finite. The fix extracts `−Δbbat` from native evaluations and
feeds the same precision-safe Taylor machinery the PINT path uses. Diagnostic
`torb_sec` in native terms uses host `torb_binary_sec` (tempo2 `obsn[i].torb`),
not that closure.
...
#### `fixed_state_bclt` — envelope reference (one-pass BCLT + full tail)
...
**single pass** — no BCLT `lax.scan` iteration. Still runs the **full tempo2 tail**
(ref + pert native evals) for `−Δbbat_sec`.
...
Use when: validating the fast-path tangent against `staged_bclt` (envelope
reference). Compare `fixed_state_stripped` against this mode before promoting
stripped for production NUTS.

#### `fixed_state_stripped` — fast PTA / NUTS target (after validation)

Same host-frozen staging as `fixed_state_bclt` (ephemeris, clocks, `dt_ssb_ref_sec`,
and **host-cached reference BBAT** day/sec from term diagnostics). In-graph physics
uses a **BBAT lite subgraph**: one perturbed native eval through formBats only —
no phase5, TRACK−2, or second ref eval. `−Δbbat_sec` is pert BBAT minus the
host-frozen reference BBAT.

- **Nonlinear:** astrometry, parallax, Shapiro, DM, and side channels (JUMP, FD, DMX, SW)
  still move with θ via the shared fitting contract.
- **Fixed:** reference BCLT epoch and reference BBAT; no BCLT scan; no full tail.
- **Tradeoff:** largest compile/eval win among production modes; tangent must pass
  envelope gates vs `fixed_state_bclt` and `staged_bclt` before IPTA NUTS promotion.

Use when: PTA NUTS / Discovery workloads **after** validation shows residual and
gradient differences vs `fixed_state_bclt` (envelope) and `staged_bclt` (fidelity)
stay below target tolerances for the parameters and step sizes of interest.
...
Tradeoff: slower compile and eval than `fixed_state_bclt` or `fixed_state_stripped`,
but preserves the self-consistent BCLT scan that tempo2 uses when parameters move.
...
|------|------------------------|
| `fixed_state_bclt` | One-pass BCLT at **frozen** `dt_ssb_ref` + full tempo2 tail (envelope reference) |
| `fixed_state_stripped` | Same host freeze + **BBAT lite** (single pert vs host ref BBAT) |
| `staged_bclt` | Host-frozen staging + **BCLT recomputed** in JAX |
| `full` | Full in-graph chain (no host freeze of clocks/ephemeris) |

Avoid calling `fixed_state_bclt` or `fixed_state_stripped` "linearized" or
"PINT-compatible." Both fast paths are still nonlinear; they omit BCLT iteration
feedback (and stripped omits the full tail), not nonlinearity in the delay terms
themselves.
...
| Residual kernel (`fixed_state_bclt`) | `jug/residuals/tempo2/model/fixed_state.py` → `compute_tempo2_toa_model_fixed_state_nonlinear_jax()` |
| BBAT lite kernel (`fixed_state_stripped`) | `jug/residuals/tempo2/model/bbat_lite.py` → `compute_bbat_lite_daysec_jax()` |
...
### Validation plan for fast graph modes

Before promoting `fixed_state_stripped` for production NUTS:
...
4. **Parity envelope (`fixed_state_bclt`)** — compare `fixed_state_bclt` vs `staged_bclt` for
   small PTA-scale perturbations (**green:** < 1 ns RMS on wsrt167,
   `test_tempo2_residual_delta_jax.py`).
5. **Stripped tangent gate** — compare `fixed_state_stripped` vs `staged_bclt`
   at PTA-scale ε (**green:** < 1 ns RMS on wsrt167; lite BBAT vs pytempo < 1 ns in
   `test_tempo2_stripped_mode.py`).

Green host residual gates on Cases A/B/C do **not** automatically certify
`fixed_state_stripped` for IPTA-scale traced likelihoods.

### Cross-family comparison

| | PINT-family JAX path | Tempo2 `staged_bclt` | Tempo2 `fixed_state_bclt` | Tempo2 `fixed_state_stripped` | Tempo2 `full` |
|---|---------------------|----------------------|---------------------------|-------------------------------|---------------|
| Host parity target | PINT (~5 ps matched inputs; tens of ns in CI) | libstempo (ns gates) | Same host freeze as staged | Same host freeze as staged | Oracle only |
| BCLT in graph | No (frozen `dt_sec`) | Yes (iterated scan) | No (one-pass at `dt_ssb_ref`) | No (one-pass at `dt_ssb_ref`) | Yes (full chain) |
| BBAT eval | PINT delay change | Full tail ×2 | Full tail ×2 | BBAT lite ×1 vs host ref | Full chain |
| Astrometry in graph | Nonlinear (PINT formulas) | Nonlinear (tempo2 native) | Nonlinear (tempo2 native) | Nonlinear (tempo2 native) | Nonlinear (tempo2 native) |
| Default for NUTS | Only option today | **Yes** (tempo2 fidelity) | Envelope reference | Validated on wsrt167; IPTA promotion pending | Never |
| Picosecond tests | JAX vs JUG NumPy only | JAX internal + staged parity | JAX vs staged envelope | JAX vs `fixed_state_bclt` envelope | Dev oracle |
...
| Design matrix in tempo2 mode | **Analytic** (default WLS): PINT-style simplified tangents via `designmatrix_assembly.py` — independent of `tempo2_native`. **Autodiff**: native `bbat` + `tempo2_native` chain for NUTS/high-fidelity columns. Gates: `tests/test_tempo2_analytic_designmatrix.py` vs simplified-model `jacfwd`. |
| Tempo2-native JAX fitting | **Graph modes** (default `staged_bclt`): `tempo2_native={staged_bclt,fixed_state_bclt,fixed_state_stripped,full}` plus `tempo2_jug_options` for IERS/BCLT/cache knobs. Requires `term_diagnostics['tempo2_obs_state']` in residual cache for `native_chain_static`. |
...
| Canonical tangent for fitting | **WLS (default):** analytic simplified tangents (`design_matrix_method="analytic"`). **NUTS / native fidelity:** tempo2 autodiff / `residual_delta_jax` when `design_matrix_method="autodiff"`. Libstempo autodiff column gates: wsrt167 F0/RAJ/DECJ/DM on staged/fixed/stripped; binary on `epta_j1909`; addsat F0 on `epta_j0613_addsat_min`. Remaining: `full` autodiff columns; full EPTA J0613 addsat multi-param. |
...
Supported `tempo2_native` values (exact strings only): `staged_bclt`, `fixed_state_bclt`, `fixed_state_stripped`, `full`.

Notebook integrators must call `compute_residuals` (or
...
| Field | Units | Use |
|-------|-------|-----|
| `bbat_mjd`, `bat_mjd`, `pet_mjd` | MJD | Epoch scalars — **not** interchangeable with `bat_corr_days`. JAX path uses split long-double-style assembly and closes pytempo to < 1 ns on wsrt167. Host Taylor `term_diagnostics['bbat_mjd']` can still show ~350 ns vs pytempo (diagnostic only). See [`PARITY_ROADMAP.md`](PARITY_ROADMAP.md) § "formBats MJD assembly". |
| `torb_sec` | seconds | tempo2 `obsn[i].torb` (binary orbit time). JAX `Tempo2Terms.torb_sec` uses host `prebinary − total`, not Taylor `dt_sec` closure. |
...
A: **Yes** for default WLS and fast `compute_designmatrix` export. Analytic columns use
PINT-style simplified geometric tangents (approximate vs native `bbat`, by design).
Use `"autodiff"` when you need the native tempo2 JAX tangent (NUTS whitening, libstempo
column parity).
...
- Use **`fixed_state_bclt`** as the envelope reference when validating fast-path
  tangents vs `staged_bclt`.
- Promote **`fixed_state_stripped`** for PTA NUTS once envelope tests vs
  `fixed_state_bclt` and fidelity checks vs `staged_bclt` pass on representative workloads.
- Reserve **`full`** for oracle checks and development (`jug/testing/DEV_ORACLE.md`).

### Notebook integrators / `export_jax_timing_state`
