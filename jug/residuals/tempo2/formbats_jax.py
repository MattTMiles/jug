...
Diagnostic ``bat_mjd`` / ``bbat_mjd`` views use compensated ``two_sum`` assembly.
"""
...
def compute_formbats_daysec(
    ...
    """Port ``formBats.C`` L67-L83 with split tempo2 ``long double`` summation."""
    other_sec = (
        correction_tt_tb_sec
        - tropospheric_sec
        + roemer_sec
    ...
    bat_int, bat_sec = add_seconds_daysec(sat_int_day, sat_sec_in_day, correction_tt_sec)
    bat_int, bat_sec = add_seconds_daysec(bat_int, bat_sec, other_sec)
    bbat_int, bbat_sec = add_seconds_daysec(bat_int, bat_sec, -shklovskii_sec)
    tt_day = correction_tt_sec / SECS_PER_DAY
    other_day = other_sec / SECS_PER_DAY
    bat_corr_day, bat_corr_resid = two_sum(tt_day, other_day)
    return (
        bat_corr_day,
        bat_corr_resid,
        bat_int,
...
def compute_shklovskii_sec_jax_pure_daysec(
    ...
    *,
    ...
    """Pure JAX Shklovskii from two-part BAT and POSEPOCH (``secularMotion.C``)."""
    dshk = jnp.asarray(dshk, dtype=jnp.float64)
    pmra = jnp.asarray(pmra, dtype=jnp.float64)
    pmdec = jnp.asarray(pmdec, dtype=jnp.float64)
    bat_sec = jnp.asarray(bat_sec, dtype=jnp.float64)
    kpc2m = 3.08568025e19
    mas_yr2rad_s = 1.536281850e-16
    t0 = (bat_int - posepoch_int) * SECS_PER_DAY + bat_sec - posepoch_frac * SECS_PER_DAY
    pm2 = (pmra * pmra + pmdec * pmdec) * mas_yr2rad_s * mas_yr2rad_s
    shk = (t0 * t0 / (2.0 * C_KM_S)) * (dshk * kpc2m) * pm2
    has_pm = (pmra != 0.0) | (pmdec != 0.0)
    active = (dshk != 0.0) & has_pm
    return jnp.where(active, shk, jnp.zeros_like(shk))
