"""Test J1012-4235 (DD binary) with DE440 ephemeris."""
...
print("="*80)
print("J1012-4235 (DD BINARY) VALIDATION TEST")
print("="*80)
...
if np.std(diff_ns) < 50:
    ...
    print("   DD binary model is working correctly!")
else:
...
ax.set_title(f'J1012-4235 (DD): JUG vs PINT Residuals')
ax.legend()
...
plt.tight_layout()
plt.savefig('j1012_dd_comparison.png', dpi=150)
print(f"Plot saved to: j1012_dd_comparison.png")

print("\n" + "="*80)
print("SUMMARY")
print("="*80)
print(f"\nJ1012-4235 (DD binary, {len(jug_res_us)} TOAs):")
print(f"  JUG RMS:  {np.std(jug_res_us):.6f} μs")
print(f"  PINT RMS: {np.std(pint_res_us):.6f} μs")
print(f"  Difference RMS: {np.std(diff_ns):.3f} ns")
if np.std(diff_ns) < 50:
    print(f"\n✅ DD model validated - working correctly!")
    print(f"   Both ELL1 and DD binary models meet the 50 ns target!")
else:
    print(f"\n⚠️  DD model still has issues - need to investigate further")
