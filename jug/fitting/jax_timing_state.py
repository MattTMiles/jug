"""Frozen JAX timing state exported from a JUG session.

``export_jax_timing_state`` snapshots a populated ``TimingSession`` into a
convention-frozen :class:`JaxTimingState`: a jitted residual-delta closure, the
reference residuals/parameters, and the linearized timing design matrix, all in
the native-delta convention. External likelihood stacks (Discovery, Enterprise,
nltiming) evaluate nonlinear timing residuals through this state without
touching JUG internals.
"""
...
def export_jax_timing_state(
    ...
    *,
    ...
    # RAJ/DECJ fit-unit conversion back to the native delta convention.
