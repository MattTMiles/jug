...
from .common import (
    ...
    tempo2_torb_binary_sec_from_td,
    track2_pulse_arrays_from_toas,
)
from .terms import compute_tempo2_terms_jax


def _torb_binary_jax_from_td(td: dict):
    torb = tempo2_torb_binary_sec_from_td(td)
    if torb is None:
        return None
    return jnp.asarray(torb, dtype=jnp.float64)
...
def build_delta_pack(setup: "GeneralFitSetup") -> NativeDeltaPack | None:
    ...
    return NativeDeltaPack(
        ...
        torb_binary_sec=_torb_binary_jax_from_td(td),
...
def _resolve_dt_ssb_ref_sec(
    ...
    *,
    ...
    if model_static.host_frozen:
        if jug_result is None or toas is None:
            raise ValueError(
                "bclt_dt_ssb_sec missing from term_diagnostics and host-frozen "
                "model_static cannot run BCLT fallback without jug_result/toas"
            )
        model_static = _load_model_static_for_native_chain(
            params,
            toas,
            jug_result,
            pulse_numbers=model_static.pulse_numbers,
            pn_add=model_static.pn_add,
            jump_phase=model_static.jump_phase,
            tzr_phase=model_static.tzr_phase,
            track_val=int(model_static.track_val),
            host_frozen=False,
        )
    tt_pre = np.asarray(
...
def _mjd_array_to_daysec(mjd: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    mjd = np.asarray(mjd, dtype=np.float64)
    int_day = np.floor(mjd)
    sec_in_day = (mjd - int_day) * SECS_PER_DAY
    return int_day, sec_in_day


def _trace_shklovskii_for_fit(fit_params: list[str] | None) -> bool:
    shk_params = {
        "PMRA",
        "PMDEC",
        "PX",
    ...
    return bool(shk_params.intersection(fit_params or []))


def _stripped_host_fields_from_td(td: dict) -> dict[str, Any]:
    """Optional host diagnostic anchors for stripped mode (not used in BBAT lite)."""
    fields: dict[str, Any] = {}
    if td.get("bat_mjd") is not None:
        bat_int, bat_sec = _mjd_array_to_daysec(np.asarray(td["bat_mjd"], dtype=np.float64))
        fields["bat_ref_int_day"] = jnp.asarray(bat_int, dtype=jnp.float64)
        fields["bat_ref_sec_in_day"] = jnp.asarray(bat_sec, dtype=jnp.float64)
    shk_ref = td.get("shklovskii_sec")
    if shk_ref is not None:
        shk_arr = np.asarray(shk_ref, dtype=np.float64)
        if shk_arr.ndim == 0:
            n = len(td["sat_mjd"])
            shk_arr = np.full(n, float(shk_arr), dtype=np.float64)
        fields["shk_ref_sec"] = jnp.asarray(shk_arr, dtype=jnp.float64)
    planet_shapiro = td.get("planet_shapiro_sec")
    if planet_shapiro is not None:
        planet_arr = np.asarray(planet_shapiro, dtype=np.float64)
        if planet_arr.ndim == 0:
            n = len(td["sat_mjd"])
            planet_arr = np.full(n, float(planet_arr), dtype=np.float64)
        fields["shapiro_planets_ref_sec"] = jnp.asarray(planet_arr, dtype=jnp.float64)
    return fields


def _lite_bbat_ref_daysec_host(params: dict, pack: NativeDeltaPack) -> tuple[jnp.ndarray, jnp.ndarray]:
    """Evaluate BBAT lite at reference parameters for stripped delta anchoring."""
    import jax

    from jug.residuals.tempo2.model.bbat_lite import bbat_lite_daysec_from_pack

    bbat_int, bbat_sec = bbat_lite_daysec_from_pack(params, pack)
    return (
        jnp.asarray(jax.device_get(bbat_int), dtype=jnp.float64),
        jnp.asarray(jax.device_get(bbat_sec), dtype=jnp.float64),
    )


def _finalize_stripped_pack(params: dict, pack: NativeDeltaPack) -> NativeDeltaPack:
    """Attach lite-built reference BBAT daysec (not host ``bbat_mjd``)."""
    bbat_int, bbat_sec = _lite_bbat_ref_daysec_host(params, pack)
    return replace(
        pack,
        bbat_ref_int_day=bbat_int,
        bbat_ref_sec_in_day=bbat_sec,
    )
...
def _build_fixed_state_pack_from_host(
    *,
    ...
    bclt_max_iter: int | None = None,
    mode: str = TEMPO2_GRAPH_FIXED_STATE_BCLT,
    stripped_fields: dict[str, Any] | None = None,
    trace_shklovskii: bool = False,
) -> NativeDeltaPack:
    ...
    pack_kwargs: dict[str, Any] = {
        ...
        "einstein_rate": jnp.asarray(einstein, dtype=jnp.float64),
        "tropo_sec": jnp.asarray(tropo, dtype=jnp.float64),
        "dt_ssb_ref_sec": jnp.asarray(dt_ssb_ref_sec, dtype=jnp.float64),
        "ne_sw": float(model_static.ne_sw),
        "use_native_ecliptic": bool(model_static.use_native_ecliptic),
        "dm_epoch": float(params.get("DMEPOCH", params["PEPOCH"])),
        "dm_coeffs_ref": _dm_coeffs_from_params(params),
    ...
    if stripped_fields:
        pack_kwargs.update(stripped_fields)
        pack_kwargs["trace_shklovskii"] = trace_shklovskii
    return NativeDeltaPack(**pack_kwargs)
...
def build_fixed_state_bclt_delta_pack(
    setup: "GeneralFitSetup",
) -> NativeDeltaPack | None:
    """Build fixed-state one-pass BCLT cache for tempo2-native residual deltas."""
    static = getattr(setup, "native_chain_static", None)
    if static is None:
        return None
    toas = static.get("toas")
...
def build_fixed_state_stripped_delta_pack(
    setup: "GeneralFitSetup",
) -> NativeDeltaPack | None:
    """Build BBAT-lite stripped cache for tempo2-native residual deltas."""
    static = getattr(setup, "native_chain_static", None)
    if static is None:
        return None
    toas = static.get("toas")
    ...
    pack = _build_fixed_state_pack_from_host(
        ...
        dt_ssb_ref_sec=dt_ssb_ref,
        bclt_max_iter=_bclt_max_iter_from_setup(setup),
        mode=TEMPO2_GRAPH_FIXED_STATE_STRIPPED,
        stripped_fields=stripped_fields,
        trace_shklovskii=_trace_shklovskii_for_fit(setup.fit_param_list),
    )
    return _finalize_stripped_pack(params, pack)
...
def build_delta_pack_for_setup(
    ...
    if mode == TEMPO2_GRAPH_FIXED_STATE_BCLT:
        return build_fixed_state_bclt_delta_pack(setup)
    if mode == TEMPO2_GRAPH_FIXED_STATE_STRIPPED:
        return build_fixed_state_stripped_delta_pack(setup)
