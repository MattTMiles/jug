...
def tempo2_torb_binary_sec_from_td(td: dict) -> np.ndarray | None:
    """Tempo2 ``obsn[i].torb`` from host delay ledger (matches pytempo oracle)."""
    pre = td.get("prebinary_delay_sec")
    total = td.get("total_delay_sec")
    if pre is None or total is None:
        return None
    return np.asarray(pre, dtype=np.float64) - np.asarray(total, dtype=np.float64)
...
class NativeDeltaPack:
    ...
    # staged_bclt / fixed_state_bclt (host-frozen staging)
    ...
    # fixed_state_stripped (BBAT lite)
    bbat_ref_int_day: Any | None = None
    bbat_ref_sec_in_day: Any | None = None
    bat_ref_int_day: Any | None = None
    bat_ref_sec_in_day: Any | None = None
    shk_ref_sec: Any | None = None
    shapiro_planets_ref_sec: Any | None = None
    trace_shklovskii: bool = False
    # Host-frozen tempo2 binary orbit time (obsn[i].torb); used for torb_sec parity.
    torb_binary_sec: Any | None = None
...
def _resolve_host_frozen_profile_metadata(
    params: dict,
    compatibility: str,
) -> tuple[object, bool]:
    """Resolve engine profile and troposphere flag without loading clock/SPK tables."""
    from jug.residuals.diagnostic_conventions import default_conventions
    from jug.residuals.engine_conventions import _flag_from_par, resolve_engine_profile
...
def _load_model_static_for_native_chain(
    ...
    *,
    ...
    """Load static inputs for the tempo2-native JAX model.

    When ``host_frozen=True`` (``staged_bclt``, ``fixed_state_bclt``,
    ``fixed_state_stripped``), only fit metadata is resolved — SPK, EOP, IFTE,
    and clock chains are omitted because geometry/clocks are host-frozen.
    """
    from jug.io.clock import resolve_clock_dir
    from jug.residuals.diagnostic_conventions import resolve_ne_sw_cm3
    from jug.residuals.tempo2.model.static import build_tempo2_model_static_host_frozen
    ...
    profile, correct_tropo = _resolve_host_frozen_profile_metadata(params, compatibility)
    from jug.residuals.diagnostic_conventions import resolve_planet_shapiro_enabled

    if host_frozen:
        return build_tempo2_model_static_host_frozen(
            params=params,
            toas=toas,
